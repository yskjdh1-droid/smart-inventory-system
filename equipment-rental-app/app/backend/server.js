...
const { initFirebase, sendPushToUser }     = require('./utils/fcm');
const {
...
// ── 라우터 ────────────────────────────────────────
app.use('/api/auth',       authRouterV2);  // 이메일 인증 포함
...
// ──────────────────────────────────────────────────
// CRON 1: 매일 자정 — 연체 상태 자동 전환 + 패널티 부과
// ──────────────────────────────────────────────────
cron.schedule('0 0 * * *', async () => {
  console.log('[CRON] 연체 처리 시작');
  const now = new Date();
  const actives = await Rental.find({ status: 'ACTIVE', dueDate: { $lt: now } }).populate('user');
  ...
  for (const r of actives) {
    ...
    if (days >= 1) {
      ...
      if (user.penaltyScore >= 10 && !user.isSuspended) {
        ...
        await sendPushToUser(user, '⚠️ 연체 알림', `${r.equipment} 반납이 ${days}일 초과됐습니다. 즉시 반납해주세요.`);
      }
    }
  }
...
// ──────────────────────────────────────────────────
// CRON 2: 매일 오전 9시 — 반납 예정 D-3, D-1 알림
// ──────────────────────────────────────────────────
cron.schedule('0 9 * * *', async () => {
  ...
  for (const days of [3, 1]) {
    ...
    const rentals = await Rental.find({ status: 'ACTIVE', dueDate: { $gte: start, $lte: end } }).populate('user').populate('equipment', 'modelName');
