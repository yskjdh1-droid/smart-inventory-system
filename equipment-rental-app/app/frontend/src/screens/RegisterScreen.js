...
export default function RegisterScreen() {
  ...
  const handleRegister = async () => {
    ...
    if (password.length > 18)
      return Alert.alert('비밀번호 오류', '비밀번호는 18자리 이하여야 합니다.');
    ...
    try {
      ...
      Alert.alert('가입 완료', '회원가입이 완료됐습니다!\n로그인해주세요.', [
      ...
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        {step === 'email' && (
          <View style={styles.card}>
            <Text style={styles.stepTitle}>이메일을 입력하세요</Text>
        ...
        {step === 'info' && (
          <View style={styles.card}>
            ...
            <Text style={styles.label}>
              비밀번호 * <Text style={styles.labelHint}>(8자 이상 18자 이하)</Text>
            </Text>
            <TextInput
            ...
            {form.password.length > 0 && (
              <Text style={[styles.pwHint, form.password.length < 8 && { color: COLORS.red }]}>
                {form.password.length}/18자
                {form.password.length < 8 ? '  ⚠️ 8자 이상 입력하세요' : '  ✓'}
              </Text>
            )}

            <Text style={[styles.label, { marginTop: 10 }]}>비밀번호 확인 *</Text>
            <TextInput
              style={[
                styles.input,
...
const styles = StyleSheet.create({
  ...
  stepRow:       { flexDirection: 'row', justifyContent: 'center', marginBottom: 24 },
  ...
  labelHint:     { fontSize: 11, color: COLORS.gray, fontWeight: '400' },
  ...
  pwHint:        { fontSize: 11, color: COLORS.gray, marginBottom: 4 },
